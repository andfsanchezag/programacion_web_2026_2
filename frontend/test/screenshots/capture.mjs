...
const report = {
  ...
  repairBackend: [
    {
      id: 'RB-1',
      need: 'GET /api/v1/commercial/customers?identification={id} -> CustomerResponseDTO 200 (o extender GET /api/v1/teller/customers al rol COMMERCIAL_EMPLOYEE)',
      reason: 'Frontend-Role-Modules §7 exige búsqueda de clientes para comercial; hoy el lookup responde 403 y el flujo opera con identificación directa',
      status: 'BLOCKED-subfeature',
    },
    {
      id: 'RB-2',
      need: 'GET /api/v1/internal-analyst/customers?identification={id} -> CustomerResponseDTO 200 (o extender teller-search al rol INTERNAL_ANALYST)',
      reason: 'Role-Modules §8: gestión de estado de clientes sin lookup previo; hoy opera con identificación directa y confirmación sobre el 200 del PATCH',
      status: 'BLOCKED-subfeature',
    },
    {
      id: 'OBS-1',
      need: 'Alinear Api-rest-endpoints.md §8.5 (documenta 201) con el comportamiento real 200 de POST /api/v1/teller/accounts (el seed backend espera 200)',
      reason: 'Sólo documentación; el frontend acepta cualquier 2xx',
      status: 'OBSERVATION',
    },
  ],
};
...
async function ready(page, timeout = 30000) {
  await page.waitForFunction(() => (document.querySelector('#root')?.children.length ?? 0) > 0, null, { timeout });
  await page.waitForTimeout(400);
}

async function selectFirstSource(page, timeout = 25000) {
  await page.waitForFunction(
    () => document.querySelectorAll('#tr-source option').length > 1,
    null,
    { timeout },
  );
  await page.locator('#tr-source').selectOption({ index: 1 });
}
...
async function main() {
  ...
  try {
    ...
    const only = process.env.SHOT_VP ?? '';
    for (const vp of VIEWPORTS.filter((v) => !only || `${v.w}x${v.h}` === only)) {
      if (!(await frontOk())) {
        report.errors.push(`viewport ${vp.w}x${vp.h}: dev server caído antes de iniciar`);
        continue;
      }
      ...
      // ---- Público: login real ----
      const ctx0 = await browser.newContext({ viewport: { width: vp.w, height: vp.h } });
      ...
      await login.unrouteAll({ behavior: 'wait' });
      await settled(login);
      ...
      // ---- Drawer móvil ----
      if (vp.w <= 480) {
        const ctxm = await browser.newContext({ viewport: { width: vp.w, height: vp.h } });
      ...
      // ---- Reduced motion ----
      const ctxr = await browser.newContext({ viewport: { width: vp.w, height: vp.h }, reducedMotion: 'reduce' });
      ...
      report.shots.push({ viewport: `${vp.w}x${vp.h}`, files });
        ...
        report.errors.push(`viewport ${vp.w}x${vp.h}: ${e.message?.slice(0, 300) ?? e}`);
      } finally {
        await browser.close();
      }
    }
    ...
    // Merge con corridas previas (cada viewport puede capturarse por separado).
    let prev = { shots: [], errors: [], a11y: {} };
    try {
      if (existsSync(`${OUT}report.json`)) prev = JSON.parse(readFileSync(`${OUT}report.json`, 'utf8'));
    } catch { /* reporte previo ilegible: se reemplaza */ }
    const vps = new Set(report.shots.map((s) => s.viewport));
    report.shots = [...prev.shots.filter((s) => !vps.has(s.viewport)), ...report.shots];
    report.errors = [...(prev.errors ?? []), ...report.errors];
    report.a11y = { ...(prev.a11y ?? {}), ...report.a11y };
    writeFileSync(`${OUT}report.json`, JSON.stringify(report, null, 2));
    console.log(`[shots] OK: ${report.shots.reduce((n, s) => n + s.files.length, 0)} capturas`);
  } finally {
