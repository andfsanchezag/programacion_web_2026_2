...
El flujo se organiza como un **ciclo de diagnóstico, implementación, containerización, reparación y validación**, con fases numeradas de `0A` a `6`. Las tareas paralelas solo se ejecutan cuando el diagnóstico confirma que sus dependencias están satisfechas y sus archivos no se solapan.
...
graph TD
  Sub0S[Fase 0: Diagnóstico de estado y selección de siguiente paso] --> Sub0A[Fase 0A: Auto-detección de stack]
    ...
    Sub0B --> Sub0C[Fase 0C: Containerización de la aplicación]
    Sub0C --> Sub1[Fase 1: Agente Dominio Core]
...
El estado debe quedar escrito en un artefacto versionable, preferiblemente `SDD/orchestrator-state.md` o `SDD/orchestrator-state.json`. Cada ejecución debe actualizarlo después de cada tarea con:
...
El diagnóstico debe evaluar **cada etapa y cada entregable**, no solo el área general. Para cada fase (`0A`, `0B`, `0C`, `1`, `2A`, `2B`, `2C`, `3A`, `3B`, `4`, `5A`, `5B` y `6`) debe registrar como mínimo:
...
### FASE 0C: Containerización de la aplicación

**Objetivo:** asegurar que la aplicación completa pueda construirse y ejecutarse en Docker, además de sus dependencias MySQL y MongoDB.
...
**Entregables obligatorios, adaptados al stack detectado:**
...
2. `.dockerignore` que excluya al menos dependencias locales, cobertura, logs, secretos, `.git`, artefactos temporales y archivos de entorno reales.
3. Actualizar `docker-compose.yml` o `compose.yaml` para incluir la aplicación, además de MySQL y MongoDB:
...
4. Healthchecks:
  ...
  - la aplicación debe comprobar `/health` o el endpoint equivalente;
...
5. Documentar en `README.md` o en la documentación del proyecto:
  ...
  - `docker compose build`, `docker compose up -d`, `docker compose ps`, logs y apagado;
...
> 4. Implementa el Global Exception Handler según `SDD/Adapters/Global-exception-handler.md`: middleware global para TypeScript/Express o `@RestControllerAdvice` para Java/Spring.
...
2. Validar la configuración y puertos reales de MySQL y MongoDB mediante Docker Compose.
3. Construir la imagen de la aplicación desde cero con `docker compose build --no-cache` o el comando equivalente documentado.
4. Levantar el entorno completo con `docker compose up -d` y comprobar que aplicación, MySQL y MongoDB estén `healthy` mediante `docker compose ps` y logs.
5. Verificar que la aplicación dentro del contenedor se conecte a MySQL y MongoDB usando nombres de servicio, no `localhost`.
6. Probar `/health` desde el host y desde la red Docker cuando sea posible.
7. Ejecutar pruebas de integración de adapters, bootstrap, cierre de conexiones y persistencia real.
8. Probar los flujos REST principales: registro, login, autorización por rol, cuentas, préstamos, transferencias y auditoría.
9. Comparar métodos, rutas, códigos HTTP, DTOs, estados y excepciones contra `SDD/Adapters/Api-rest-endpoints.md`.
10. Validar cada endpoint documentado y sus principales errores contra `SDD/Adapters/Global-exception-handler.md`.
...
11. **Global Exception Handler:** existe un handler global registrado, cumple `SDD/Adapters/Global-exception-handler.md`, mapea las categorías HTTP, devuelve el formato uniforme y tiene pruebas de errores conocidos y desconocidos.
